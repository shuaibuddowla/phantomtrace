...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/shuaibuddowla/phantomtrace.git
   cd phantomtrace
   ```

2. **Firebase Configuration:**
   Because `google-services.json` contains private API keys and project identifiers, it is excluded from this repository. You must add your own:
   - Go to the [Firebase Console](https://console.firebase.google.com/) and create a new project.
   - Add an Android App with the package name `com.example.phantomtracing`.
   - Download your project's `google-services.json`.
   - Place `google-services.json` inside the `app/` directory of your cloned project:
     ```text
     phantomtrace/
     └── app/
